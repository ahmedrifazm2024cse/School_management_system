  ...
  const checkConnection = async () => {
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        setError('Connection timeout. Backend server may be slow or not running.');
      } else {
        setError('Backend server not running. Please run START_FULL.bat first.');
      }
    } finally {
      setIsChecking(false);
    }
  ...
  if (isConnected) {
    return (
      <Box sx={{ position: 'fixed', top: 10, right: 10, zIndex: 1000 }}>
        <Chip 
          icon={<CheckCircle />} 
          label={`Connected ${lastChecked ? lastChecked.toLocaleTimeString() : ''}`}
          color="success" 
          size="small"
          variant="outlined"
        />
